...
def work_oz_fbo_stocks(schema):
    oz_fbo_stocks_new = pd.read_excel(mwm.path_FD + 'oz_fbo_stocks_new.xlsx')
    ...
    for row in list(oz_fbo_stocks_new['warehouse_name'].unique()):
        if len(oz_fbo_stocks_new[oz_fbo_stocks_new['warehouse_name']==row])\
           - len(list(set(oz_fbo_stocks_new[oz_fbo_stocks_new['warehouse_name']==row]['item_code']))) != 0:
            ...
            exit()
    mwm.update_or_create_table_date(oz_fbo_stocks_new, 
                                    'oz_fbo_stocks',
...
with DAG(
    dag_id='oz_fbo_stocks_dag',
    default_args=mwm.args,
    schedule_interval=const.sched_int['oz_fbo_stocks_dag'],
    ...
    get_oz_fbo_stocks_new_task = PythonOperator(
                                                task_id = 'get_oz_fbo_stocks_new',
                                                python_callable=mwm.get_oz_fbo_stocks_new,
                                                dag=dag
                                                )
    
    work_oz_fbo_stocks_task = PythonOperator(
                                             task_id = 'work_oz_fbo_stocks',
                                             python_callable=work_oz_fbo_stocks,
                                             op_kwargs={'schema':mwm.ir_schema},
                                             dag=dag
                                             )
    

    get_oz_fbo_stocks_new_task >> work_oz_fbo_stocks_task