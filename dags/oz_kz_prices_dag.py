...
def work_oz_kz_prices(schema):
    oz_kz_price_new = pd.read_excel(mwm.path_FD + 'oz_kz_price_new.xlsx',
                                    dtype={'offer_id': 'str'}).fillna('')
    if len(oz_kz_price_new) == 0:
        print('В oz_kz_price_new нет данных')
        exit()
    mwm.unique_items(oz_kz_price_new.rename(columns={'offer_id': 'Артикул'}))
    mwm.update_or_create_table_date(oz_kz_price_new, 
                                    'oz_kz_prices', 
                                    ['offer_id', 'date'],
                                    'date',
                                    schema=schema)
...
with DAG(
    dag_id='oz_kz_prices_dag',
    default_args=mwm.args,
    schedule_interval=const.sched_int['oz_kz_prices_dag'],
    ...
    get_oz_kz_price_new_task = PythonOperator(
                                              task_id = 'get_oz_kz_price_new',
                                              python_callable=mwm.get_oz_price_new,
                                              op_kwargs={'account':'KZ'},
                                              dag=dag
                                             )
    
    work_oz_kz_prices_task = PythonOperator(
                                            task_id = 'work_oz_kz_prices',
                                            python_callable=work_oz_kz_prices,
                                            op_kwargs={'schema':mwm.ir_schema},
                                            dag=dag
                                           )
    
    get_oz_kz_price_new_task >> work_oz_kz_prices_task