...
def work_wb_fbs_stocks(schema):
    wb_fbs_stocks_new = pd.read_excel(mwm.path_FD + 'wb_fbs_stocks_new.xlsx',
                                      dtype={'Баркод': 'int'})
...
with DAG(
    dag_id='wb_fbs_stocks_dag',
    default_args=mwm.args,
    schedule_interval=const.sched_int['wb_fbs_stocks_dag'],
    ...
    work_wb_fbs_stocks_task = PythonOperator(
                                             task_id = 'work_wb_fbs_stocks',
                                             python_callable=work_wb_fbs_stocks,
                                             op_kwargs={'schema':mwm.ir_schema},
                                             dag=dag
                                             )
    

    get_wb_fbs_stocks_new_task >> work_wb_fbs_stocks_task