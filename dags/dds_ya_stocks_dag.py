...
def update_dds_ya_stocks(schema,
                         dds_schema):
    ...
    for account in [const.account_ir]:
        ...
        FROM {dds_schema}.dds_ya_stocks
        WHERE account = '{account.lower()}'
        order by date desc
        LIMIT 10;
        """

        list_date_dds_ya_stocks_acc = list(mwm.select_table_from_request(req=req,
                                                                         table_name=None,
                                                                         schema=dds_schema)[0])
        for dt in [dt for dt in res if dt not in list_date_dds_ya_stocks_acc]:
            try:
                ya_stocks = dm.get_redact_ya_stocks(account=account,
                                                    schema=schema,
                                                    date_filter=dt)               
            except:
            ...
            if len(ya_stocks) > 0:
                try:
                    mwm.insert_values(data=ya_stocks, 
                                      table_name='dds_ya_stocks', 
                                      schema=dds_schema)
                except:
                    print(f'Данные за {dt} не были добавлены, account {account}')
                    exit()
            else:
                print(f'Данных за {dt} аккаунта {account} нет в dds_ya_stocks')
                exit()
                
                
with DAG(
         dag_id='dds_ya_stocks_dag',
         default_args=mwm.args,
         schedule_interval=const.sched_int['dds_ya_stocks_dag'],
    ...
    update_dds_ya_stocks_task = PythonOperator(
                                               task_id='update_dds_ya_stocks',
                                               python_callable=update_dds_ya_stocks,
