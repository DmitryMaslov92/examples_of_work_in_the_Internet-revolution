...
def work_wb_fbo_stocks(schema):
    wb_fbo_stocks_new = pd.read_excel(mwm.path_FD + 'wb_fbo_stocks_new.xlsx',
...
with DAG(
    dag_id='wb_fbo_stocks_dag',
    default_args=mwm.args,
    schedule_interval=const.sched_int['wb_fbo_stocks_dag'],
    ...
    work_wb_fbo_stocks_task = PythonOperator(
                                             task_id = 'work_wb_fbo_stocks',
                                             python_callable=work_wb_fbo_stocks,
                                             op_kwargs={'schema':mwm.ir_schema},
                                             dag=dag
                                             )
    

    get_wb_fbo_stocks_new_task >> work_wb_fbo_stocks_task