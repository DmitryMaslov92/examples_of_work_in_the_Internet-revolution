...
# Проверить таблицу oz_fbs_orders в БД на пропущенные дни за последние 10 дней
def check_oz_fbs_orders(ti, 
                           end_day, 
                           start_day):
    ...
    if len(df[0].unique()) != 10:
        print('В таблице oz_fbs_orders пропущены дни, нужно обновить')
...
def get_oz_fbs_orders_new_p1(ti,
                             end_day, 
                             start_day):
    update_marker = ti.xcom_pull(key='update_marker', task_ids=['check_oz_fbs_orders'])[0]
    if update_marker == True:
        try:
            dm.get_oz_fbs_orders_new(account='IR',
...
def update_oz_fbs_orders_new_p1(ti,
    ...
    if update_marker == True:
        ...
        if get_marker_1 == True:
            try:
                oz_fbs_orders_new = pd.read_excel(mwm.path_FD + 'oz_ir_fbs_orders_new.xlsx').fillna(0)
            except:
                print('Загрузка oz_fbs_orders_new завершилась с ошибкой')
                exit()
            if oz_fbs_orders_new.empty == True:
                print('Датафрейм пустой')
                exit()
            dt_value_df = ti.xcom_pull(key='dt_value', task_ids=['check_oz_fbs_orders'])[0].split(', ')
            for dt in [y.strftime('%d-%B-%Y') for y in 
                       sorted([pd.to_datetime(x) for x in list(oz_fbs_orders_new['in_process_at'].unique())])]:
                if dt not in dt_value_df:
                    mwm.insert_values(data=oz_fbs_orders_new[oz_fbs_orders_new['in_process_at']==dt]\
                                           .reset_index(drop=True), 
                                      table_name='oz_fbs_orders', 
                                      schema=schema)
        else:
            None
    else:
...
def update_oz_fbs_orders_new_p2(ti,
    ...
    if get_marker_2 == True:
        ...
        for dt in [y.strftime('%d-%B-%Y') for y in 
                   sorted([pd.to_datetime(x) for x in list(oz_fbs_orders_new['in_process_at'].unique())])]:
            try:
                mwm.update_data_date_filter(data=oz_fbs_orders_new[oz_fbs_orders_new['in_process_at']==dt]\
                                                 .reset_index(drop=True),
                                            table_name='oz_fbs_orders',
...
with DAG(
         dag_id='oz_fbs_orders_dag',
         default_args=mwm.args,
         schedule_interval=const.sched_int['oz_fbs_orders_dag'],
    ...
    check_oz_fbs_orders_task = PythonOperator(
                                                 task_id='check_oz_fbs_orders',
                                                 python_callable=check_oz_fbs_orders,
