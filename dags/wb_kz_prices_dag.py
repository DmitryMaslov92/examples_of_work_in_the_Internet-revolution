...
def work_wb_kz_prices(schema):
    wb_price_new = pd.read_excel(mwm.path_FD + 'wb_kz_price_new.xlsx',
                                 dtype={'Баркод':'int'})
...
with DAG(
    dag_id='wb_kz_prices_dag',
    default_args=mwm.args,
    schedule_interval=const.sched_int['wb_kz_prices_dag'],
    ...
    work_wb_kz_prices_task = PythonOperator(
                                         task_id = 'work_wb_kz_prices',
                                         python_callable=work_wb_kz_prices,
                                         op_kwargs={'schema':mwm.ir_schema},
                                         dag=dag
                                        )
    

    get_wb_price_new_task >> work_wb_kz_prices_task